 ...
 **/
static enum e_storage_add storage_canAddItem(struct s_storage *stor, int idx, Item items[], int amount, int max_num) {
 ...
 */
static int storage_additem(MapSessionData* sd, struct s_storage *stor, Item *it, int amount)
{
...
void storage_guild_log( MapSessionData* sd, Item* item, int16 amount ){
	int i;
...
enum e_guild_storage_log storage_guild_log_read_sub( MapSessionData* sd, std::vector<struct guild_log_entry>& log, uint32 max ){
	StringBuf buf;
	int j;
...
bool storage_guild_additem(MapSessionData* sd, struct s_storage* stor, Item* ItemData, int amount)
{
	...
	if(ItemData->nameid == 0 || amount <= 0)
		return false;

	id = itemdb_search(ItemData->nameid);

	if( id->stack.guild_storage && amount > id->stack.amount ) // item stack limitation
		return false;

	if (!itemdb_canguildstore(ItemData, pc_get_group_level(sd)) || ItemData->expire_time) { // Check if item is storable. [Skotlex]
		clif_displaymessage (sd->fd, msg_txt(sd,264));
		return false;
	}

	if ((ItemData->bound == BOUND_ACCOUNT || ItemData->bound > BOUND_GUILD) && !pc_can_give_bounded_items(sd)) {
		clif_displaymessage(sd->fd, msg_txt(sd,294));
		return false;
	}
	...
	if(itemdb_isstackable2(id)) { //Stackable
		for(i = 0; i < stor->max_amount; i++) {
			if(compare_item(&stor->u.items_guild[i], ItemData)) {
	...
	memcpy(&stor->u.items_guild[i],ItemData,sizeof(stor->u.items_guild[0]));
	stor->u.items_guild[i].amount = amount;
	stor->amount++;
	clif_storageitemadded(sd,&stor->u.items_guild[i],i,amount);
	clif_updatestorageamount(sd, stor->amount, stor->max_amount);
