...
typedef bool (*searchstore_search_t)(MapSessionData* sd, t_itemid nameid);
typedef bool (*searchstore_searchall_t)(MapSessionData* sd, const struct s_search_store_search* s);
 ...
 * @return : store type
 */
static bool searchstore_hasstore(MapSessionData* sd, unsigned char type)
{
	switch( type ) {
 ...
 * @return : store ID
 */
static int searchstore_getstoreid(MapSessionData* sd, unsigned char type)
{
	switch( type ) {
...
void searchstore_query(MapSessionData* sd, unsigned char type, unsigned int min_price, unsigned int max_price, const struct PACKET_CZ_SEARCH_STORE_INFO_item* itemlist, unsigned int item_count, const struct PACKET_CZ_SEARCH_STORE_INFO_item* cardlist, unsigned int card_count)
{
	...
	for( pl_sd = (MapSessionData*)dbi_first(iter); dbi_exists(iter);  pl_sd = (MapSessionData*)dbi_next(iter) ) {
