...
bool status_check_visibility(const block_list* src, const block_list* target, bool checkblind)
{
	...
	switch (src->type) {
		case BL_MOB:
			view_range = reinterpret_cast<const mob_data*>(src)->db->range3;
			break;
		case BL_PET:
			view_range = reinterpret_cast<const pet_data*>(src)->db->range2;
			break;
		default:
	...
	if (tsc) {
		...
		switch (target->type) {	// Check for chase-walk/hiding/cloaking opponents.
			case BL_PC: {
					const map_session_data *tsd = reinterpret_cast<const map_session_data*>(target);
...
const char* status_get_name( const block_list& bl ){
	switch( bl.type ){
		case BL_PC: {
				const map_session_data& sd = reinterpret_cast<const map_session_data&>( bl );
		...
		case BL_MOB:
			return reinterpret_cast<const mob_data&>( bl ).name;

		case BL_PET:
			return reinterpret_cast<const pet_data&>( bl ).pet.name;

		case BL_HOM:
			return reinterpret_cast<const homun_data&>( bl ).homunculus.name;

		case BL_MER:
			// They only have database names which are global, not specific to GID.
			return reinterpret_cast<const s_mercenary_data&>( bl ).db->name.c_str();

		case BL_NPC:
			return reinterpret_cast<const npc_data&>( bl ).name;

		case BL_ELEM:
			// They only have database names which are global, not specific to GID.
			return reinterpret_cast<const s_elemental_data&>( bl ).db->name.c_str();
	}
...
int32 status_get_class(const block_list *bl)
{
	nullpo_ret(bl);
	switch( bl->type ) {
		case BL_PC:		return reinterpret_cast<const map_session_data*>(bl)->status.class_;
		case BL_MOB:	return reinterpret_cast<const mob_data*>(bl)->vd->look[LOOK_BASE]; // Class used on all code should be the view class of the mob.
		case BL_PET:	return reinterpret_cast<const pet_data*>(bl)->pet.class_;
		case BL_HOM:	return reinterpret_cast<const homun_data*>(bl)->homunculus.class_;
		case BL_MER:	return reinterpret_cast<const s_mercenary_data*>(bl)->mercenary.class_;
		case BL_NPC:	return reinterpret_cast<const npc_data*>(bl)->class_;
		case BL_ELEM:	return reinterpret_cast<const s_elemental_data*>(bl)->elemental.class_;
	}
	return 0;
}
...
uint16 status_get_speed( const block_list* bl )
{
	...
	if(bl->type==BL_NPC)// Only BL with speed data but no status_data [Skotlex]
		return (reinterpret_cast<const npc_data*>(bl))->speed;
...
int32 status_get_party_id( const block_list *bl )
{
	...
	switch (bl->type) {
		case BL_PC:
			return reinterpret_cast<const map_session_data*>(bl)->status.party_id;
		case BL_PET:
			if (reinterpret_cast<const pet_data*>(bl)->master)
				return reinterpret_cast<const pet_data*>(bl)->master->status.party_id;
			break;
		case BL_MOB: {
		...
		case BL_HOM:
			if (reinterpret_cast<const homun_data*>(bl)->master)
				return reinterpret_cast<const homun_data*>(bl)->master->status.party_id;
			break;
		case BL_MER:
			if (reinterpret_cast<const s_mercenary_data*>(bl)->master)
				return reinterpret_cast<const s_mercenary_data*>(bl)->master->status.party_id;
			break;
		case BL_SKILL:
			if (reinterpret_cast<const skill_unit*>(bl)->group)
				return reinterpret_cast<const skill_unit*>(bl)->group->party_id;
			break;
		case BL_ELEM:
			if (reinterpret_cast<const s_elemental_data*>(bl)->master)
				return reinterpret_cast<const s_elemental_data*>(bl)->master->status.party_id;
			break;
	}
	return 0;
}
...
int32 status_get_guild_id(const block_list *bl)
{
	...
	switch (bl->type) {
		case BL_PC:
			return reinterpret_cast<const map_session_data*>(bl)->status.guild_id;
		case BL_PET:
			if (reinterpret_cast<const pet_data*>(bl)->master)
				return reinterpret_cast<const pet_data*>(bl)->master->status.guild_id;
			break;
		case BL_MOB:
			{
				map_session_data *msd;
		...
		case BL_HOM:
			if (reinterpret_cast<const homun_data*>(bl)->master)
				return reinterpret_cast<const homun_data*>(bl)->master->status.guild_id;
			break;
		case BL_MER:
			if (reinterpret_cast<const s_mercenary_data*>(bl)->master)
				return reinterpret_cast<const s_mercenary_data*>(bl)->master->status.guild_id;
			break;
		case BL_NPC:
			if (reinterpret_cast<const npc_data*>(bl)->subtype == NPCTYPE_SCRIPT)
				return reinterpret_cast<const npc_data*>(bl)->u.scr.guild_id;
			break;
		case BL_SKILL:
			if (reinterpret_cast<const skill_unit*>(bl)->group)
				return reinterpret_cast<const skill_unit*>(bl)->group->guild_id;
			break;
		case BL_ELEM:
			if (reinterpret_cast<const s_elemental_data*>(bl)->master)
				return reinterpret_cast<const s_elemental_data*>(bl)->master->status.guild_id;
			break;
	}
	return 0;
}
...
int32 status_get_emblem_id(const block_list *bl)
{
	...
	switch (bl->type) {
		case BL_PC:
			return reinterpret_cast<const map_session_data*>(bl)->guild_emblem_id;
		case BL_PET:
			if (reinterpret_cast<const pet_data*>(bl)->master)
				return reinterpret_cast<const pet_data*>(bl)->master->guild_emblem_id;
			break;
		case BL_MOB:
			{
		...
		case BL_HOM:
			if (reinterpret_cast<const homun_data*>(bl)->master)
				return reinterpret_cast<const homun_data*>(bl)->master->guild_emblem_id;
			break;
		case BL_MER:
			if (reinterpret_cast<const s_mercenary_data*>(bl)->master)
				return reinterpret_cast<const s_mercenary_data*>(bl)->master->guild_emblem_id;
			break;
		case BL_NPC:
			if (reinterpret_cast<const npc_data*>(bl)->subtype == NPCTYPE_SCRIPT && reinterpret_cast<const npc_data*>(bl)->u.scr.guild_id > 0) {
				auto g = guild_search(reinterpret_cast<const npc_data*>(bl)->u.scr.guild_id);
				if (g)
		...
		case BL_ELEM:
			if (reinterpret_cast<const s_elemental_data*>(bl)->master)
				return reinterpret_cast<const s_elemental_data*>(bl)->master->guild_emblem_id;
			break;
}
	return 0;
}
...
std::vector<e_race2> status_get_race2(const block_list *bl)
{
	...
	if (bl->type == BL_MOB)
		return reinterpret_cast<const mob_data*>(bl)->db->race2;
	if (bl->type == BL_PET)
		return reinterpret_cast<const pet_data*>(bl)->db->race2;
...
int32 status_isimmune(const block_list *bl)
{
	...
	if (bl->type == BL_PC &&
		(reinterpret_cast<const map_session_data*>(bl))->special_state.no_magic_damage >= battle_config.gtb_sc_immunity)
		return (reinterpret_cast<const map_session_data*>(bl))->special_state.no_magic_damage;
	return 0;
}
...
bool status_isendure(const block_list& bl, t_tick tick, bool visible)
{
	...
	if (bl.m >= 0 && bl.type == BL_PC && !status_change_isDisabledOnMap(SC_ENDURE, map_getmapdata(bl.m))) {
		if (reinterpret_cast<const map_session_data&>(bl).special_state.no_walk_delay)
			return true;
	}
...
t_tick status_get_sc_def(const block_list *src, const block_list *bl, sc_type type, int32 rate, t_tick tick, uint8 flag)
{
	...
	sd = reinterpret_cast<const map_session_data*>(bl);
	const status_data* status = status_get_status_data(*bl);
...
int32 status_change_end( block_list* bl, enum sc_type type, int32 tid ){
	...
	switch(type) {
		...
		case SC_WINKCHARM:
			if (bl->type == BL_MOB) {
				mob_unlocktarget(reinterpret_cast<mob_data*>(bl), gettick());
			}
			break;
		case SC_TENSIONRELAX:
