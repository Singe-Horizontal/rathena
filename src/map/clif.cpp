...
static inline bool disguised(const block_list* bl) {
	return (bool)( bl->type == BL_PC && reinterpret_cast<const map_session_data*>(bl)->disguise );
}
...
int32 clif_send(const void* buf, int32 len, const block_list* bl, enum send_target type)
{
	...
	case ALL_CLIENT: //All player clients.
		...
		while( ( tsd = (reinterpret_cast<const map_session_data*>(mapit_next( iter )))) != nullptr ){
...
static void clif_set_unit_walking( const block_list& bl, const map_session_data* tsd, const unit_data& ud, enum send_target target ){
	...
	if( bl.type == BL_MOB ){
		p.isBoss = reinterpret_cast<const mob_data*>( &bl )->get_bosstype();
	}else if( bl.type == BL_PET ){
		p.isBoss = reinterpret_cast<const pet_data*>( &bl )->db->get_bosstype();
	}else{
		p.isBoss = BOSSTYPE_NONE;
	}
#endif
...
int32 clif_spawn( const block_list *bl, bool walking ){
	...
	case BL_PET:
		...
		if( vd->look[LOOK_HEAD_BOTTOM] != 0 ){
			const pet_data& pd = *reinterpret_cast<const pet_data*>( bl );
...
static void clif_item_normal( int16 idx, struct NORMALITEM_INFO *p, const struct item *i, const struct item_data *id ){
	nullpo_retv(p);
	nullpo_retv(i);
	nullpo_retv(id);
...
int32 clif_outsight(block_list *bl,va_list ap)
{
	...
	if (clif_session_isValid(tsd)) { //tsd has lost sight of the bl object.
		...
		case BL_PC:
			...
			break;
		case BL_ITEM:
			clif_clearflooritem( *reinterpret_cast<const flooritem_data*>( bl ), tsd );
			break;
		case BL_SKILL:
			clif_clearchar_skillunit( *reinterpret_cast<const skill_unit*>( bl ), *tsd );
			break;
		case BL_NPC:
			if( !clif_npc_mayapurple( *bl ) ){
				clif_clearunit_single( bl->id, CLR_OUTSIGHT, *tsd );
...
void clif_devotion( const block_list* src, const map_session_data* tsd ) {
	...
	WBUFL(buf,2) = src->id;
	if( src->type == BL_MER ){
		const s_mercenary_data *md = reinterpret_cast<const s_mercenary_data*>(src);
...
void clif_spiritball( const block_list *bl, const block_list* target, enum send_target send_target ){
	...
	switch( bl->type ){
		case BL_PC:
			p.num = reinterpret_cast<const map_session_data*>(bl)->spiritball;
			break;
		case BL_HOM:
			p.num = reinterpret_cast<const homun_data*>(bl)->homunculus.spiritball;
			break;
	}
...
void clif_name( const block_list* src, const block_list *bl, send_target target ){
	...
	switch( bl->type ){
		...
		case BL_ELEM: {
			...
			switch (bl->type) {
				case BL_HOM:
					memcpy(packet.name, reinterpret_cast<const homun_data*>(bl)->homunculus.name, NAME_LENGTH);
					break;
				case BL_MER:
					memcpy(packet.name, reinterpret_cast<const s_mercenary_data*>(bl)->db->name.c_str(), NAME_LENGTH);
					break;
				case BL_PET:
					safestrncpy(packet.name, reinterpret_cast<const pet_data*>(bl)->pet.name, NAME_LENGTH);
					break;
				case BL_NPC:
					safestrncpy(packet.name, reinterpret_cast<const npc_data*>(bl)->name, NAME_LENGTH);
					break;
				case BL_ELEM:
					safestrncpy(packet.name, reinterpret_cast<const s_elemental_data*>(bl)->db->name.c_str(), NAME_LENGTH);
					break;
			}
...
void clif_changed_dir(const block_list& bl, enum send_target target){
	...
	p.srcId = bl.id;
	if( bl.type == BL_PC ){
		p.headDir = reinterpret_cast<const map_session_data*>(&bl)->head_dir;
	}else{
		p.headDir = 0;
	}
