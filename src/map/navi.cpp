...
void write_spawn(std::ostream &os, const struct map_data * m, const std::shared_ptr<mobs::s_mob_db> mobinfo, int amount, int idx) {
...
void write_object_lists() {
	...
	for (int mapid = 0; mapid < map_num; mapid++) {
		...
		for (int mobidx = 0; mobidx < MAX_MOB_LIST_PER_MAP; mobidx++) {
			...
			const auto mobinfo = mobs::mob_db.find(m->moblist[mobidx]->id);
			if (mobinfo == nullptr)
				continue;
