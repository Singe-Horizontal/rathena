...
static bool script_rid2sd_( struct script_state *st, MapSessionData** sd, const char *func );
 ...
 **/
static bool script_accid2sd_(struct script_state *st, uint8 loc, MapSessionData **sd, const char *func) {
 ...
 * @return True if `sd` is assigned, false otherwise
 **/
static bool script_charid2sd_(struct script_state *st, uint8 loc, MapSessionData **sd, const char *func) {
	if (script_hasdata(st, loc)) {
		int id_ = script_getnum(st, loc);
 ...
 * @return True if `sd` is assigned, false otherwise
 **/
static bool script_nick2sd_(struct script_state *st, uint8 loc, MapSessionData **sd, const char *func) {
	if (script_hasdata(st, loc)) {
 ...
 * @return True if `sd` is assigned, false otherwise
 **/
static bool script_mapid2sd_(struct script_state *st, uint8 loc, MapSessionData **sd, const char *func) {
	if (script_hasdata(st, loc)) {
		int id_ = script_getnum(st, loc);
		if (!(*sd = map_id2sd(id_))){
 ...
 */
struct script_data *get_val_(struct script_state* st, struct script_data* data, MapSessionData *sd)
{
	const char* name;
	char prefix;
	char postfix;
 ...
 **/
void script_array_ensure_zero(struct script_state *st, MapSessionData *sd, int64 uid, struct reg_db *ref)
{
 ...
 **/
unsigned int script_array_size(struct script_state *st, MapSessionData *sd, const char *name, struct reg_db *ref)
{
 ...
 **/
unsigned int script_array_highest_key(struct script_state *st, MapSessionData *sd, const char *name, struct reg_db *ref)
{
	struct script_array *sa = NULL;
	struct reg_db *src = script_array_src(st, sd, name, ref);
 ...
 **/
struct reg_db *script_array_src(struct script_state *st, MapSessionData *sd, const char *name, struct reg_db *ref)
{
 ...
 *------------------------------------------*/
bool set_reg_str( struct script_state* st, MapSessionData* sd, int64 num, const char* name, const char* value, struct reg_db *ref ){
	char prefix = name[0];
	size_t vlen = 0;
...
bool set_reg_num( struct script_state* st, MapSessionData* sd, int64 num, const char* name, int64 value, struct reg_db *ref ){
	char prefix = name[0];
	size_t vlen = 0;
...
bool set_var_str( MapSessionData* sd, const char* name, const char* val ){
	return set_reg_str( NULL, sd, reference_uid( add_str( name ), 0 ), name, val, NULL );
}

bool clear_reg( struct script_state* st, MapSessionData* sd, int64 num, const char* name, struct reg_db *ref ){
	if( is_string_variable( name ) ){
...
void setd_sub_num( struct script_state* st, MapSessionData* sd, const char* varname, int elem, int64 value, struct reg_db* ref ){
	set_reg_num( st, sd, reference_uid( add_str( varname ), elem ), varname, value, ref );
}

void setd_sub_str( struct script_state* st, MapSessionData* sd, const char* varname, int elem, const char* value, struct reg_db* ref ){
 ...
 * @param sd
 */
const char* conv_str_(struct script_state* st, struct script_data* data, MapSessionData *sd)
{
	char* p;
 ...
 * @param sd
 */
int64 conv_num_(struct script_state* st, struct script_data* data, MapSessionData *sd)
{
	get_val_(st, data, sd);
	if( data_isint(data) )
	{// nothing to convert
...
void script_free_state(struct script_state* st)
{
	if (idb_exists(st_db, st->id)) {
		MapSessionData *sd = st->rid ? map_id2sd(st->rid) : NULL;
...
void script_run_autobonus(const char *autobonus, MapSessionData *sd, unsigned int pos)
{
...
void script_run_petautobonus(const std::string &autobonus, MapSessionData &sd) {
...
void script_cleararray_pc( MapSessionData* sd, const char* varname ){
	struct script_array *sa = NULL;
	struct reg_db *src = NULL;
...
void script_setarray_pc(MapSessionData* sd, const char* varname, uint32 idx, int64 value, int* refcache)
{
	int key;
 ...
 **/
void script_reg_destroy_single(MapSessionData *sd, int64 reg, struct script_reg_state *data)
{
...
static int buildin_viewpointmap_sub(BlockList *bl, va_list ap) {
	...
	clif_viewpoint((MapSessionData *)bl, oid, type, x, y, id, color);
	return 0;
}
 ...
 **/
static bool script_getitem_randomoption(struct script_state *st, MapSessionData* sd, Item *it, const char *funcname, int x) {
	int i, opt_id_n;
 ...
 */
static int script_countitem_sub(Item *items, std::shared_ptr<ItemData> id, int size, int expanded, struct script_state *st, MapSessionData *sd = nullptr, bool rental = false) {
...
BUILDIN_FUNC(getitem2)
{
	...
	if( script_isstring(st, 2) ) {
		...
		ItemData = item_db.searchname( name );
	...
	if( ItemData ) {
		if( ItemData->type == IT_WEAPON || ItemData->type == IT_ARMOR || ItemData->type == IT_SHADOWGEAR ) {
			if(ref > MAX_REFINE)
				ref = MAX_REFINE;
		}
		else if( ItemData->type == IT_PETEGG ) {
			iden = 1;
			ref = 0;
		}
		else {
...
BUILDIN_FUNC(rentitem) {
	...
	{
		const char *name = script_getstr(st, 2);
		std::shared_ptr<ItemData> itd = item_db.searchname( name );
...
BUILDIN_FUNC(getnameditem)
{
	...
	if( script_isstring(st, 2) ){
		const char *name = script_getstr(st, 2);
		std::shared_ptr<ItemData> ItemData = item_db.searchname( name );
...
static void buildin_delitem_delete(MapSessionData* sd, int idx, int* amount, uint8 loc, bool delete_items)
{
	int delamount;
	Item *itm = NULL;
...
static bool buildin_delitem_search(MapSessionData* sd, Item* it, uint8 exact_match, uint8 loc)
{
...
BUILDIN_FUNC(sc_end)
{
	...
	if (type > SC_NONE && type < SC_MAX) {
		...
		struct StatusChangeEntry *sce = sc->getSCE(type);
...
static int buildin_maprespawnguildid_sub_pc(MapSessionData* sd, va_list ap)
{
...
static int buildin_maprespawnguildid_sub_mob(BlockList *bl,va_list ap)
{
	...
	if(!md->GuardianData && md->mob_id != MOBID_EMPERIUM && ( !mobs::IsClone(md->mob_id) || battle_config.guild_maprespawn_clones ))
		StatusKill(bl);
...
{
	std::shared_ptr<ItemData> i_data;
...
BUILDIN_FUNC(getitemslots)
{
	...
	std::shared_ptr<ItemData> i_data = item_db.find(item_id);
...
BUILDIN_FUNC(petloot)
{
	...
	pd->loot->item = (Item *)aCalloc(max,sizeof(struct Item));
...
static int playBGM_foreachpc_sub(MapSessionData* sd, va_list args)
{
 ...
 *-----------------------------------*/
int recovery_sub(MapSessionData* sd, int revive)
{
...
BUILDIN_FUNC(addmonsterdrop)
{
	...
	std::shared_ptr<ItemData> itm = item_db.find(item_id);
	...
	mobs::ReloadItemMobData(); // Reload the mob search data stored in the ItemData
...
BUILDIN_FUNC(searchitem)
{
	...
	std::map<t_itemid, std::shared_ptr<ItemData>> items;
...
BUILDIN_FUNC(setunitdata)
{
	...
	case BL_MOB:
		...
		if (!md->base_status) {
			...
			memcpy(md->base_status, &md->db->status, sizeof(struct StatusData));
		}
...
BUILDIN_FUNC(unitattack)
{
	...
	switch(unit_bl->type) {
		case BL_PC: {
			MapSessionData* sd = (MapSessionData*)unit_bl;
...
///
/// unitStopAttack_sub <unit_id>;
BUILDIN_FUNC(unitStopAttack_sub)
{
	struct BlockList* bl;
...
BUILDIN_FUNC(unitstopwalk)
{
	...
	if(script_rid2bl(2,bl)) {
		...
		if (units::StopWalking(bl, flag) == 0 && flag != USW_FORCE_STOP) {
...
BUILDIN_FUNC(bg_monster_set_team)
{
	...
	units::StopAttack(&md->bl);
	units::StopWalking(&md->bl,0);
...
BUILDIN_FUNC(consumeitem)
{
	...
	run_script( ItemData->script, 0, sd->bl.id, 0 );
	return SCRIPT_CMD_SUCCESS;
}
...
BUILDIN_FUNC(mergeitem2) {
	...
	if (script_hasdata(st, 2)) {
		if (script_isstring(st, 2)) {// "<item name>"
			const char *name = script_getstr(st, 2);
			std::shared_ptr<ItemData> id = item_db.searchname( name );
...
BUILDIN_FUNC(getvar) {
	...
	MapSessionData *sd = map_charid2sd(char_id);
	struct script_data *data = NULL;
	const char *name = NULL;
...
static inline bool mail_sub( struct script_state *st, struct script_data *data, MapSessionData *sd, int i, const char **out_name, unsigned int *start, unsigned int *end, int32 *id ){
	const char *name;
...
BUILDIN_FUNC(mail){
	...
	while( script_hasdata(st,7) ){
		...
		for( i = 0; i < num_items && start < end; i++, start++ ){
			std::shared_ptr<ItemData> itm = item_db.find(msg.item[i].nameid);
...
BUILDIN_FUNC( laphine_synthesis ){
	...
	if( script_hasdata( st, 2 ) ){
		if( script_isstring( st, 2 ) ){
			...
			std::shared_ptr<ItemData> item = item_db.searchname( item_name );
...
BUILDIN_FUNC(mesitemlink){
	t_itemid nameid = script_getnum( st, 2 );
	std::shared_ptr<ItemData> data = item_db.find( nameid );
...
struct script_function buildin_func[] = {
	...
	BUILDIN_DEF(unitStopAttack_sub,"i"),
