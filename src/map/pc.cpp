...
#include "storage.hpp"
#include "unit.hpp" // units::StopAttack(), StopWalking()
...
static inline bool pc_attendance_rewarded_today( MapSessionData* sd );
...
* @return Group ID
*/
int pc_get_group_id(MapSessionData *sd) {
	return sd->group_id;
}
...
* @return Group Level
*/
int pc_get_group_level(MapSessionData *sd) {
...
{
	MapSessionData *sd = (MapSessionData *)data;
 ...
 */
void pc_set_bg_queue_timer(MapSessionData *sd) {
	nullpo_retv(sd);
 ...
 * @param sd: Player data
 */
void pc_delete_bg_queue_timer(MapSessionData *sd) {
	nullpo_retv(sd);
...
static TIMER_FUNC(pc_invincible_timer){
	...
	if( (sd=(MapSessionData *)map_id2sd(id)) == NULL || sd->bl.type!=BL_PC )
		return 1;
...
void pc_setinvincibletimer(MapSessionData* sd, int val) {
	nullpo_retv(sd);
...
* @param max
*/
void pc_addspiritball(MapSessionData *sd,int interval,int max)
{
	int tid;
	uint8 i;
...
*/
void pc_delspiritball(MapSessionData *sd,int count,int type)
{
	uint8 i;
 ...
 */
int pc_addsoulball(MapSessionData *sd, int max)
{
	nullpo_ret(sd);
 ...
 */
int pc_delsoulball(MapSessionData *sd, int count, bool type)
{
	nullpo_ret(sd);
...
*/
void pc_addservantball( MapSessionData& sd, int count ){
...
* @param count: Amount to remove
*/
void pc_delservantball( MapSessionData& sd, int count ){
	sd.servantball = cap_value( sd.servantball - count, 0, MAX_SERVANTBALL );
...
* @param amount: Amount to add
*/
void pc_addabyssball( MapSessionData& sd, int count ){
...
* @param count: Amount to remove
*/
void pc_delabyssball( MapSessionData& sd, int count ){
	sd.abyssball = cap_value( sd.abyssball - count, 0, MAX_ABYSSBALL );
...
*/
bool pc_addfame(MapSessionData &sd, int count)
{
...
*/
void pc_setrestartvalue(MapSessionData *sd, char type) {
	struct StatusData *status, *b_status;
	nullpo_retv(sd);
 ...
 * @param sd: Player data
 */
void pc_inventory_rental_clear(MapSessionData *sd)
{
 ...
 * @param sd: Player data
 */
void pc_inventory_rentals(MapSessionData *sd)
{
	int i, c = 0;
 ...
 */
void pc_inventory_rental_add(MapSessionData *sd, unsigned int seconds)
{
 ...
 */
bool pc_can_sell_item(MapSessionData *sd, Item *item, enum npc_subtype shoptype) {
	...
	switch (shoptype) {
		...
		case NPCTYPE_ITEMSHOP:
			if (item->bound && battle_config.allow_bound_sell&ISR_BOUND && (
				...
				struct ItemData *itd = itemdb_search(item->nameid);
 ...
 */
bool pc_can_give_items(MapSessionData *sd)
{
 ...
 */
bool pc_can_give_bounded_items(MapSessionData *sd)
{
 ...
 */
bool pc_can_trade_item(MapSessionData *sd, int index) {
	if (sd && index >= 0) {
 ...
 * @param sd
 *------------------------------------------*/
void pc_makesavestatus(MapSessionData *sd) {
	nullpo_retv(sd);
 ...
 * Off init ? Connection?
 *------------------------------------------*/
void pc_setnewpc(MapSessionData *sd, uint32 account_id, uint32 char_id, int login_id1, t_tick client_tick, int sex, int fd) {
	nullpo_retv(sd);
...
* @param id
*/
int pc_equippoint_sub(MapSessionData *sd,struct ItemData* id){
	int ep = 0;
...
/**
 * Fill inventory_data with struct *ItemData through inventory (fill with struct *item)
 * @param sd : player session
 * @return 0 sucess, 1:invalid sd
 */
void pc_setinventorydata(MapSessionData *sd)
{
	uint8 i;
	nullpo_retv(sd);
...
* @param sd : Player
*/
void pc_calcweapontype(MapSessionData *sd)
{
	nullpo_retv(sd);
...
* @param sd : Player
*/
void pc_setequipindex(MapSessionData *sd)
{
	uint16 i;
 ...
 * @return 1:yes, 0:no
 */
bool pc_isequipped(MapSessionData *sd, t_itemid nameid)
{
	uint8 i;
 ...
 */
enum adopt_responses pc_try_adopt(MapSessionData *p1_sd, MapSessionData *p2_sd, MapSessionData *b_sd)
{
	if( !p1_sd || !p2_sd || !b_sd )
 ...
 * Adoption Process
 *------------------------------------------*/
bool pc_adoption(MapSessionData *p1_sd, MapSessionData *p2_sd, MapSessionData *b_sd)
{
	int job, joblevel;
	t_exp jobexp;
...
static bool pc_job_can_use_item( MapSessionData* sd, struct ItemData* item ){
 ...
 * Credits:
		...
		[Cydh] finishing :D
 *------------------------------------------*/
static bool pc_isItemClass (MapSessionData *sd, struct ItemData* item) {
	while (1) {
 ...
 *------------------------------------------------*/
uint8 pc_isequip(MapSessionData *sd,int n)
{
 ...
 */
bool pc_lastpoint_special( MapSessionData& sd ){
 ...
 *------------------------------------------*/
bool pc_authok(MapSessionData *sd, uint32 login_id2, time_t expiration_time, int group_id, struct mmo_charstatus *st, bool changing_mapservers)
{
	t_tick tick = gettick();
 ...
 *------------------------------------------*/
void pc_authfail(MapSessionData *sd)
{
	clif_authfail_fd(sd->fd, 0);
	return;
}
 ...
 */
bool pc_set_hate_mob(MapSessionData *sd, int pos, BlockList *bl)
{
	int class_;
 ...
 *------------------------------------------*/
void pc_reg_received(MapSessionData *sd)
{
	uint8 i;
...
static bool pc_grant_allskills(MapSessionData *sd, bool addlv) {
 ...
 * @param sd
 *------------------------------------------*/
void pc_calc_skilltree(MapSessionData *sd)
{
	nullpo_retv(sd);
...
static void pc_check_skilltree(MapSessionData *sd)
{
	int flag = 0;
...
void pc_clean_skilltree(MapSessionData *sd)
{
	uint16 i;
...
uint64 pc_calc_skilltree_normalize_job_sub( MapSessionData *sd ){
...
uint64 pc_calc_skilltree_normalize_job( MapSessionData *sd ){
 ...
 */
void pc_updateweightstatus(MapSessionData *sd)
{
	int old_overweight;
	int new_overweight;
...
int pc_disguise(MapSessionData *sd, int class_)
{
 ...
 */
static void pc_bonus_autospell(std::vector<AutoSpell> &spell, uint16 id, uint16 lv, short rate, short battle_flag, t_itemid card_id, uint8 flag)
{
 ...
 *              &2: random skill level in [1..lv] is chosen
 */
static void pc_bonus_autospell_onskill(std::vector<AutoSpell> &spell, uint16 src_skill, uint16 id, uint16 lv, short rate, t_itemid card_id, uint8 flag)
{
	if (spell.size() == MAX_PC_BONUS) {
 ...
 */
static void pc_bonus_addeff(std::vector<AddEff> &effect, enum sc_type sc, int rate, short arrow_rate, unsigned char flag, unsigned int duration)
{
 ...
 * @param duration: Duration. If 0 use default duration lookup for associated skill with level 7
 */
static void pc_bonus_addeff_onskill(std::vector<AddEffOnSkill> &effect, enum sc_type sc, int rate, short skill_id, unsigned char target, unsigned int duration)
{
	if (effect.size() == MAX_PC_BONUS) {
 ...
 * @param drop: Player's sd->add_drop (struct AddDrop)
 ...
 */
static void pc_bonus_item_drop(std::vector<AddDrop> &drop, t_itemid nameid, uint16 group, int class_, short race, int rate)
{
	if (!nameid && !group) {
 ...
 */
void pc_delautobonus(MapSessionData &sd, std::vector<std::shared_ptr<s_autobonus>> &bonus, bool restore){
 ...
 */
void pc_exeautobonus(MapSessionData &sd, std::vector<std::shared_ptr<s_autobonus>> *bonus, std::shared_ptr<s_autobonus> autobonus)
{
 ...
 */
static void pc_bonus_addele(MapSessionData* sd, unsigned char ele, short rate, short flag)
{
	nullpo_retv(sd);

	struct WeaponData *wd = (sd->state.lr_flag ? &sd->left_weapon : &sd->right_weapon);
 ...
 * @param flag: Battle flag
 */
static void pc_bonus_subele(MapSessionData* sd, unsigned char ele, short rate, short flag)
{
	nullpo_retv(sd);
 ...
 * @param flag: Battle flag
*/
static void pc_bonus_subrace(MapSessionData* sd, unsigned char race, short rate, short flag)
{
 ...
 */
static void pc_bonus_itembonus(std::vector<ItemBonus> &bonus, uint16 id, int val, bool cap_rate)
{
	for (auto &it : bonus) {
		if (it.id == id) {
			if (cap_rate)
 ...
 *------------------------------------------*/
void pc_bonus(MapSessionData *sd,int type,int val)
{
 ...
 * @param val Value that usually for rate or fixed value
 *------------------------------------------*/
void pc_bonus2(MapSessionData *sd,int type,int type2,int val)
{
	nullpo_retv(sd);
	...
	case SP_ADDEFF: // bonus2 bAddEff,eff,n;
		...
		pc_bonus_addeff(sd->add_eff, (sc_type)type2, sd->state.lr_flag != 2 ? val : 0, sd->state.lr_flag == 2 ? val : 0, 0, 0);
		break;
	case SP_ADDEFF2: // bonus2 bAddEff2,eff,n;
		PC_BONUS_CHK_SC(type2,SP_ADDEFF2);
		pc_bonus_addeff(sd->add_eff, (sc_type)type2, sd->state.lr_flag != 2 ? val : 0, sd->state.lr_flag == 2 ? val : 0, ATF_SELF, 0);
		break;
	...
	case SP_ADDEFF_WHENHIT: // bonus2 bAddEffWhenHit,eff,n;
		...
		if(sd->state.lr_flag != 2)
			pc_bonus_addeff(sd->add_eff_atked, (sc_type)type2, val, 0, 0, 0);
		break;
 ...
 * @param val Value that usually for rate or fixed value
 */
void pc_bonus3(MapSessionData *sd,int type,int type2,int type3,int val)
{
	nullpo_retv(sd);
	...
	case SP_ADDEFF: // bonus3 bAddEff,eff,n,y;
		PC_BONUS_CHK_SC(type2,SP_ADDEFF);
		pc_bonus_addeff(sd->add_eff, (sc_type)type2, sd->state.lr_flag != 2 ? type3 : 0, sd->state.lr_flag == 2 ? type3 : 0, val, 0);
		break;

	case SP_ADDEFF_WHENHIT: // bonus3 bAddEffWhenHit,eff,n,y;
		PC_BONUS_CHK_SC(type2,SP_ADDEFF_WHENHIT);
		if(sd->state.lr_flag != 2)
			pc_bonus_addeff(sd->add_eff_atked, (sc_type)type2, type3, 0, val, 0);
		break;

	case SP_ADDEFF_ONSKILL: // bonus3 bAddEffOnSkill,sk,eff,n;
		PC_BONUS_CHK_SC(type3,SP_ADDEFF_ONSKILL);
		if( sd->state.lr_flag != 2 )
			pc_bonus_addeff_onskill(sd->add_eff_on_skill, (sc_type)type3, val, type2, ATF_TARGET, 0);
		break;
 ...
 * @param val Value that usually for rate or fixed value
 */
void pc_bonus4(MapSessionData *sd,int type,int type2,int type3,int type4,int val)
{
	nullpo_retv(sd);
	...
	case SP_ADDEFF: // bonus4 bAddEff,eff,n,y,t;
		PC_BONUS_CHK_SC(type2,SP_ADDEFF);
		pc_bonus_addeff(sd->add_eff, (sc_type)type2, sd->state.lr_flag != 2 ? type3 : 0, sd->state.lr_flag == 2 ? type3 : 0, type4, val);
		break;

	case SP_ADDEFF_WHENHIT: // bonus4 bAddEffWhenHit,eff,n,y,t;
		PC_BONUS_CHK_SC(type2,SP_ADDEFF_WHENHIT);
		if (sd->state.lr_flag != 2)
			pc_bonus_addeff(sd->add_eff_atked, (sc_type)type2, type3, 0, type4, val);
		break;

	case SP_ADDEFF_ONSKILL: // bonus4 bAddEffOnSkill,sk,eff,n,y;
		PC_BONUS_CHK_SC(type3,SP_ADDEFF_ONSKILL);
		if( sd->state.lr_flag != 2 )
			pc_bonus_addeff_onskill(sd->add_eff_on_skill, (sc_type)type3, type4, type2, val, 0);
		break;
...
* @param val Value that usually for rate or fixed value
*/
void pc_bonus5(MapSessionData *sd,int type,int type2,int type3,int type4,int type5,int val)
{
	nullpo_retv(sd);
	...
	case SP_ADDEFF_ONSKILL: // bonus5 bAddEffOnSkill,sk,eff,n,y,t;
		PC_BONUS_CHK_SC(type3,SP_ADDEFF_ONSKILL);
		if( sd->state.lr_flag != 2 )
			pc_bonus_addeff_onskill(sd->add_eff_on_skill, (sc_type)type3, type4, type2, type5, val);
		break;
 ...
 *------------------------------------------*/
bool pc_skill(MapSessionData* sd, uint16 skill_id, int level, enum e_addskill_type type) {
	uint16 idx = 0;
	nullpo_ret(sd);
 ...
 * @return True on success or false otherwise
 */
bool pc_skill_plagiarism(MapSessionData &sd, uint16 skill_id, uint16 skill_lv)
{
 ...
 * @return True on success or false otherwise
 */
bool pc_skill_plagiarism_reset(MapSessionData &sd, uint8 type)
{
	uint16 idx;
	if (type == 1) 
 ...
 *------------------------------------------*/
int pc_insert_card(MapSessionData* sd, int idx_card, int idx_equip)
{
	nullpo_ret(sd);
	...
	int i;
	t_itemid nameid;
	struct ItemData* item_eq = sd->inventory_data[idx_equip];
	struct ItemData* item_card = sd->inventory_data[idx_card];
 ...
 */
int pc_identifyall(MapSessionData *sd, bool identify_item)
{
 ...
 *------------------------------------------*/
int pc_modifybuyvalue(MapSessionData *sd,int orig_value)
{
 ...
 *------------------------------------------*/
int pc_modifysellvalue(MapSessionData *sd,int orig_value)
{
 ...
 *------------------------------------------*/
char pc_checkadditem(MapSessionData *sd, t_itemid nameid, int amount)
{
	int i;
 ...
 *------------------------------------------*/
uint8 pc_inventoryblank(MapSessionData *sd)
{
	uint16 i;
	uint8 b;
 ...
 */
char pc_payzeny(MapSessionData *sd, int zeny, enum e_log_pick_type type, uint32 log_charid)
{
	nullpo_retr(2,sd);
 ...
 */
char pc_getzeny(MapSessionData *sd, int zeny, enum e_log_pick_type type, uint32 log_charid)
{
	nullpo_retr(-1,sd);
 ...
 */
int pc_paycash(MapSessionData *sd, int price, int points, e_log_pick_type type)
{
	int cash;
	nullpo_retr(-1,sd);
 ...
 */
int pc_getcash(MapSessionData *sd, int cash, int points, e_log_pick_type type)
{
	char output[CHAT_SIZE_MAX];
 ...
 **/
short pc_search_inventory(MapSessionData *sd, t_itemid nameid) {
	short i;
	nullpo_retr(-1, sd);
 ...
 *   7 = stack limitation
 */
enum e_additem_result pc_additem(MapSessionData *sd,Item *item,int amount,e_log_pick_type log_type) {
	struct ItemData *id;
	int16 i;
	unsigned int w;
 ...
 *------------------------------------------*/
char pc_delitem(MapSessionData *sd,int n,int amount,int type, short reason, e_log_pick_type log_type)
{
	nullpo_retr(1, sd);
 ...
 *------------------------------------------*/
bool pc_dropitem(MapSessionData *sd,int n,int amount)
{
	nullpo_retr(1, sd);
 ...
 * @return False = fail; True = success
 *------------------------------------------*/
bool pc_takeitem(MapSessionData *sd,struct flooritem_data *fitem)
{
	int flag = 0;
	t_tick tick = gettick();
	...
	if (fitem->first_get_charid > 0 && fitem->first_get_charid != sd->status.char_id) {
		MapSessionData *first_sd = map_charid2sd(fitem->first_get_charid);
		if (DIFF_TICK(tick,fitem->first_get_tick) < 0) {
			...
			MapSessionData *second_sd = map_charid2sd(fitem->second_get_charid);
			if (DIFF_TICK(tick, fitem->second_get_tick) < 0) {
				...
				MapSessionData *third_sd = map_charid2sd(fitem->third_get_charid);
 ...
 *	true = yes
 *------------------------------------------*/
bool pc_isUseitem(MapSessionData *sd,int n)
{
	struct ItemData *item;
	t_itemid nameid;
 ...
 */
enum e_additem_result pc_cart_additem(MapSessionData *sd,Item *item,int amount,e_log_pick_type log_type)
{
	struct ItemData *data;
	int i,w;
 ...
 *------------------------------------------*/
void pc_cart_delitem(MapSessionData *sd,int n,int amount,int type,e_log_pick_type log_type)
{
	nullpo_retv(sd);
 ...
 *------------------------------------------*/
void pc_putitemtocart(MapSessionData *sd,int idx,int amount)
{
	nullpo_retv(sd);
	...
	Item *ItemData = &sd->inventory.u.items_inventory[idx];

	if( ItemData->nameid == 0 || amount < 1 || ItemData->amount < amount || sd->state.vending || sd->state.prevend )
		return;

	if( ItemData->equipSwitch ){
		clif_msg( sd, C_ITEM_EQUIP_SWITCH );
		return;
	}

	enum e_additem_result flag = pc_cart_additem(sd,ItemData,amount,LOG_TYPE_NONE);
 ...
 *------------------------------------------*/
int pc_cartitem_amount(MapSessionData* sd, int idx, int amount)
{
	Item* ItemData;

	nullpo_retr(-1, sd);

	ItemData = &sd->cart.u.items_cart[idx];
	if( ItemData->nameid == 0 || ItemData->amount == 0 )
		return -1;
 ...
 *------------------------------------------*/
void pc_getitemfromcart(MapSessionData *sd,int idx,int amount)
{
	nullpo_retv(sd);
	...
	Item *ItemData=&sd->cart.u.items_cart[idx];

	if (ItemData->nameid == 0 || amount < 1 || ItemData->amount < amount || sd->state.vending || sd->state.prevend)
		return;

	enum e_additem_result flag = pc_additem(sd, ItemData, amount, LOG_TYPE_NONE);
 ...
 *------------------------------------------*/
int pc_show_steal(BlockList *bl,va_list ap)
{
	MapSessionData *sd;
	t_itemid itemid;
	char output[100];
 ...
 * @return True on success or false otherwise
 */
bool pc_steal_item(MapSessionData *sd,BlockList *bl, uint16 skill_lv)
{
	int i;
	t_itemid itemid;
	double rate;
	unsigned char flag = 0;
	struct StatusData *sd_status, *md_status;
	mobs::MobData *md;
 ...
 *	1 = success
 *------------------------------------------*/
int pc_steal_coin(MapSessionData *sd,BlockList *target)
{
	int rate, target_lv;
	mobs::MobData *md;
 ...
 *------------------------------------------*/
enum e_setpos pc_setpos(MapSessionData* sd, unsigned short mapindex, int x, int y, clr_type clrtype)
{
	...
	if( sd->state.changemap ) { // Misc map-changing settings
			...
			for (const auto &it : status_db) {
				if (sc->getSCE(it.first)) {
					...
					if (it.second->flag[SCF_RESTARTONMAPWARP] && it.second->skill_id > 0) {
						StatusChangeEntry *sce = sd->sc.getSCE(it.first);
		...
		if( battle_config.loose_ap_on_map && mapdata_flag_vs( mapdata ) ){
			StatusPercentDamage( nullptr, &sd->bl, 0, 0, 100, 0 );
		}
...
enum e_setpos pc_setpos_savepoint( MapSessionData& sd, clr_type clrtype ){
	struct map_data *mapdata = map_getmapdata( sd.bl.m );
 ...
 *	1,2,3 = Fail
 *------------------------------------------*/
char pc_randomwarp(MapSessionData *sd, clr_type type, bool ignore_mapflag)
{
	int x,y,i=0;
 ...
 *------------------------------------------*/
bool pc_memo(MapSessionData* sd, int pos)
{
	int skill;
 ...
 */
int pc_get_skillcooldown(MapSessionData *sd, uint16 skill_id, uint16 skill_lv) {
 ...
 *------------------------------------------*/
uint8 pc_checkskill(MapSessionData *sd, uint16 skill_id)
{
	uint16 idx = 0;
	if (sd == NULL)
		return 0;
 ...
 */
e_skill_flag pc_checkskill_flag(MapSessionData &sd, uint16 skill_id) {
	uint16 idx;
 ...
 */
uint8 pc_checkskill_summoner(MapSessionData *sd, e_summoner_power_type type) {
	if (sd == nullptr)
		return 0;
 ...
 */
uint8 pc_checkskill_imperial_guard(MapSessionData *sd, short flag)
{
	nullpo_retr(0, sd);
 ...
 */
static void pc_checkallowskill(MapSessionData *sd)
{
	nullpo_retv(sd);
 ...
 * Return
 * -1 : Nothing equipped
 * idx : (this index could be used in inventory to found ItemData)
 *------------------------------------------*/
short pc_checkequip(MapSessionData *sd,int pos, bool checkall)
{
	uint8 i;
 ...
 * -return true,false
 *------------------------------------------*/
bool pc_checkequip2(MapSessionData *sd, t_itemid nameid, int min, int max)
{
	int i;
...
int pc_stop_following (MapSessionData *sd)
{
	nullpo_ret(sd);
	...
	units::StopWalking(&sd->bl, 1);

	return 0;
}

int pc_follow(MapSessionData *sd,int target_id)
{
	BlockList *bl = map_id2bl(target_id);
...
void pc_baselevelchanged(MapSessionData *sd) {
	uint8 i;
	for( i = 0; i < EQI_MAX; i++ ) {
...
*/
static void pc_calcexp(MapSessionData *sd, t_exp *base_exp, t_exp *job_exp, BlockList *src)
{
 ...
 **/
void pc_gainexp_disp(MapSessionData *sd, t_exp base_exp, t_exp next_base_exp, t_exp job_exp, t_exp next_job_exp, bool lost) {
	char output[CHAT_SIZE_MAX];
 ...
 * @return
 **/
void pc_gainexp(MapSessionData *sd, BlockList *src, t_exp base_exp, t_exp job_exp, uint8 exp_flag)
{
 ...
 **/
void pc_lostexp(MapSessionData *sd, t_exp base_exp, t_exp job_exp) {
 ...
 * @return Max Base Level
 **/
unsigned int pc_maxbaselv(MapSessionData *sd){
 ...
 * @return Max Job Level
 **/
unsigned int pc_maxjoblv(MapSessionData *sd){
 ...
 **/
bool pc_is_maxbaselv(MapSessionData *sd) {
	nullpo_retr(false, sd);
 ...
 * @return True if reached max level
 **/
bool pc_is_maxjoblv(MapSessionData *sd) {
	nullpo_retr(false, sd);
 ...
 **/
t_exp pc_nextbaseexp(MapSessionData *sd){
	nullpo_ret(sd);
 ...
 **/
t_exp pc_nextjobexp(MapSessionData *sd){
	nullpo_ret(sd);
...
int pc_getstat(MapSessionData *sd, int type)
{
	nullpo_retr(-1, sd);
...
int pc_setstat(MapSessionData* sd, int type, int val)
{
	nullpo_retr(-1, sd);
...
int pc_need_status_point(MapSessionData* sd, int type, int val)
{
 ...
 */
int pc_maxparameterincrease(MapSessionData* sd, int type)
{
 ...
 */
bool pc_statusup(MapSessionData* sd, int type, int increase)
{
...
int pc_need_trait_point(MapSessionData* sd, int type, int val)
{
	nullpo_retr(0, sd);
...
* @return Maximum value the stat could grow by.
*/
int pc_maxtraitparameterincrease(MapSessionData* sd, int type)
{
	nullpo_ret(sd);
...
* @retval false if there were no changes.
*/
bool pc_traitstatusup(MapSessionData* sd, int type, int increase)
{
	nullpo_ret(sd);
...
* @retval 0 if no changes were made.
*/
int pc_traitstatusup2(MapSessionData* sd, int type, int val)
{
	nullpo_ret(sd);
 ...
 *------------------------------------------*/
void pc_skillup(MapSessionData *sd,uint16 skill_id)
{
	uint16 idx = skill_get_index(skill_id);
 ...
 * /allskill
 *------------------------------------------*/
int pc_allskillup(MapSessionData *sd)
{
	int i;
 ...
 *------------------------------------------*/
int pc_resetskill(MapSessionData* sd, int flag)
{
	int i, skill_point=0;
	nullpo_ret(sd);
 ...
 * /resetfeel [Komurka]
 *------------------------------------------*/
int pc_resetfeel(MapSessionData* sd)
{
	int i;
	nullpo_ret(sd);
...
int pc_resethate(MapSessionData* sd)
{
	int i;
	nullpo_ret(sd);
...
int pc_skillatk_bonus(MapSessionData *sd, uint16 skill_id)
{
	int bonus = 0;
...
int pc_sub_skillatk_bonus(MapSessionData *sd, uint16 skill_id)
{
	int bonus = 0;
...
int pc_skillheal_bonus(MapSessionData *sd, uint16 skill_id) {
...
int pc_skillheal2_bonus(MapSessionData *sd, uint16 skill_id) {
...
void pc_respawn(MapSessionData* sd, clr_type clrtype)
{
	if( !pc_isdead(sd) )
 ...
 *------------------------------------------*/
void pc_damage(MapSessionData *sd,BlockList *src,unsigned int hp, unsigned int sp, unsigned int ap)
{
 ...
 */
void pc_close_npc(MapSessionData *sd,int flag)
{
	nullpo_retv(sd);
 ...
 *------------------------------------------*/
int pc_dead(MapSessionData *sd,BlockList *src)
{
	int i=0,k=0;
	t_tick tick = gettick();
	struct map_data *mapdata = map_getmapdata(sd->bl.m);
	...
	for(k = 0; k < MAX_DEVOTION; k++) {
		if (sd->devotion[k]){
			MapSessionData *devsd = map_id2sd(sd->devotion[k]);
			if (devsd)
	...
	for (k = 0; k < MAX_STELLAR_MARKS; k++) {
		if (sd->stellar_mark[k]) {
			MapSessionData *smarksd = map_id2sd(sd->stellar_mark[k]);
	...
	for (k = 0; k < MAX_UNITED_SOULS; k++) {
		if (sd->united_soul[k]) {
			MapSessionData *usoulsd = map_id2sd(sd->united_soul[k]);
	...
	for (k = 0; k < MAX_SERVANT_SIGN; k++) {
		if (sd->servant_sign[k]) {
			MapSessionData *ssignsd = map_id2sd(sd->servant_sign[k]);
	...
	switch (src->type) {
		case BL_MOB:
		{
			...
			if(battle_config.mobs_level_up && md->status.hp &&
				...
				!md->GuardianData && !md->special_state.ai// Guardians/summons should not level. [Skotlex]
			) { 	// monster level up [Valaris]
				clif_misceffect(&md->bl,0);
				md->level++;
				status_calc_mob(md, SCO_NONE);
				StatusPercentHeal(src,10,0);
	...
	if (src && src->type == BL_PC) {
		MapSessionData *ssd = (MapSessionData *)src;
...
void pc_revive(MapSessionData *sd,unsigned int hp, unsigned int sp, unsigned int ap) {
	if(hp) clif_updatestatus(sd,SP_HP);
	if(sp) clif_updatestatus(sd,SP_SP);
	if(ap) clif_updatestatus(sd,SP_AP);
...
bool pc_revive_item(MapSessionData *sd) {
	nullpo_retr(false, sd);
 ...
 *------------------------------------------*/
int64 pc_readparam(MapSessionData* sd,int64 type)
{
	int64 val = 0;
 ...
 *------------------------------------------*/
bool pc_setparam(MapSessionData *sd,int64 type,int64 val_tmp)
{
	nullpo_retr(false,sd);
 ...
 *------------------------------------------*/
void pc_heal(MapSessionData *sd,unsigned int hp,unsigned int sp, unsigned int ap, int type)
 ...
 */
int pc_itemheal(MapSessionData *sd, t_itemid itemid, int hp, int sp)
{
...
int pc_percentheal(MapSessionData *sd,int hp,int sp)
{
	...
	if(hp >= 0 && sp >= 0) //Heal
		return StatusPercentHeal(&sd->bl, hp, sp);
	...
	if(hp <= 0 && sp <= 0) //Damage (negative rates indicate % of max rather than current), and only kill target IF the specified amount is 100%
		return StatusPercentDamage(NULL, &sd->bl, hp, sp, hp==-100);

	//Crossed signs
	if(hp) {
		if(hp > 0)
			StatusPercentHeal(&sd->bl, hp, 0);
		else
			StatusPercentDamage(NULL, &sd->bl, hp, 0, hp==-100);
	}

	if(sp) {
		if(sp > 0)
			StatusPercentHeal(&sd->bl, 0, sp);
		else
			StatusPercentDamage(NULL, &sd->bl, 0, sp, false);
	}
	return 0;
}

static int jobchange_killclone(BlockList *bl, va_list ap)
{
	mobs::MobData *md;
		int flag;
	md = (mobs::MobData *)bl;
 ...
 **/
bool pc_jobchange(MapSessionData *sd,int job, char upper)
{
	int i, fame_flag = 0;
 ...
 *------------------------------------------*/
void pc_equiplookall(MapSessionData *sd)
{
	nullpo_retv(sd);
 ...
 *------------------------------------------*/
void pc_changelook(MapSessionData *sd,int type,int val) {
	nullpo_retv(sd);
 ...
 *------------------------------------------*/
void pc_setoption(MapSessionData *sd,int type, int subtype)
{
 ...
 **/
bool pc_setcart(MapSessionData *sd,int type) {
#ifndef NEW_CARTS
 ...
 *------------------------------------------*/
void pc_setfalcon(MapSessionData* sd, int flag)
{
	if( flag ){
 ...
 *  Set player riding
 *------------------------------------------*/
void pc_setriding(MapSessionData* sd, int flag)
{
 ...
 */
void pc_setmadogear(MapSessionData *sd, bool flag, e_mado_type type)
{
 ...
 *------------------------------------------*/
bool pc_candrop(MapSessionData *sd, Item *item)
{
 ...
 *------------------------------------------*/
int64 pc_readreg(MapSessionData* sd, int64 reg)
{
 ...
 *------------------------------------------*/
bool pc_setreg(MapSessionData* sd, int64 reg, int64 val)
{
 ...
 *------------------------------------------*/
char* pc_readregstr(MapSessionData* sd, int64 reg)
{
 ...
 *------------------------------------------*/
bool pc_setregstr(MapSessionData* sd, int64 reg, const char* str)
{
	struct script_reg_str *p = NULL;
 ...
 **/
int64 pc_readregistry(MapSessionData *sd, int64 reg)
{
 ...
 **/
char* pc_readregistry_str(MapSessionData *sd, int64 reg)
{
	struct script_reg_str *p = NULL;
 ...
 * - '##type' (permanent numeric account reg2)
 **/
bool pc_setregistry(MapSessionData *sd, int64 reg, int64 val)
{
	struct script_reg_num *p = NULL;
 ...
 * - '##type$' (permanent str account reg2)
 **/
bool pc_setregistry_str(MapSessionData *sd, int64 reg, const char *val)
{
	struct script_reg_str *p = NULL;
	const char *regname = get_str(script_getvarid(reg));
	unsigned int index = script_getvaridx(reg);
 ...
 * @return True if success, false if failed.
 **/
bool pc_setreg2(MapSessionData *sd, const char *reg, int64 val) {
	char prefix = reg[0];
 ...
 **/
int64 pc_readreg2(MapSessionData *sd, const char *reg) {
	char prefix = reg[0];
 ...
 *------------------------------------------*/
bool pc_addeventtimer(MapSessionData *sd,int tick,const char *name)
{
	int i;
	nullpo_retr(false,sd);
 ...
 *------------------------------------------*/
bool pc_deleventtimer(MapSessionData *sd,const char *name)
{
	char* p = NULL;
	int i;
 ...
 *------------------------------------------*/
void pc_addeventtimercount(MapSessionData *sd,const char *name,int tick)
{
	int i;
 ...
 *------------------------------------------*/
void pc_cleareventtimer(MapSessionData *sd)
{
	int i;
 ...
 */
static int pc_checkcombo(MapSessionData *sd, ItemData *data) {
	int success = 0;
 ...
 */
static int pc_removecombo(MapSessionData *sd, ItemData *data ) {
 ...
 */
int pc_load_combo(MapSessionData *sd) {
	int ret = 0;
	...
	for (int16 i = 0; i < EQI_MAX; i++) {
		...
		if (!itemdb_isspecial(sd->inventory.u.items_inventory[idx].card[0])) {
			for (uint8 j = 0; j < MAX_SLOTS; j++) {
				...
				std::shared_ptr<ItemData> data = item_db.find(sd->inventory.u.items_inventory[idx].card[j]);
 ...
 * return: false - fail; true - success
 *------------------------------------------*/
bool pc_equipitem(MapSessionData *sd,short n,int req_pos,bool equipswitch)
{
	...
	if(pos == EQP_ACC) { //Accessories should only go in one of the two.
		...
		for (i = 0; i < sd->inventory_data[n]->slots; i++) { // Accessories that have cards that force equip location
			...
			std::shared_ptr<ItemData> card_data = item_db.find(sd->inventory.u.items_inventory[n].card[i]);
	...
	if (itemdb_isspecial(sd->inventory.u.items_inventory[n].card[0]))
		...
		for (i = 0; i < MAX_SLOTS; i++) {
			...
			std::shared_ptr<ItemData> data = item_db.find(sd->inventory.u.items_inventory[n].card[i]);
 ...
 */
static void pc_unequipitem_sub(MapSessionData *sd, int n, int flag) {
	int i, iflag;
 ...
 * @return True on success or false on failure
 */
bool pc_unequipitem(MapSessionData *sd, int n, int flag) {
	int i, pos;
...
int pc_equipswitch( MapSessionData* sd, int index ){
...
void pc_equipswitch_remove( MapSessionData* sd, int index ){
 ...
 *------------------------------------------*/
void pc_checkitem(MapSessionData *sd) {
	int i, calc_flag = 0;
	Item* it;
 ...
 *   4 - Storage
 *------------------------------------------*/
void pc_check_available_item(MapSessionData *sd, uint8 type)
{
	int i;
	t_itemid nameid;
	char output[256];
 ...
 *------------------------------------------*/
static int pc_calc_pvprank_sub(BlockList *bl,va_list ap)
{
	MapSessionData *sd1,*sd2;

	sd1=(MapSessionData *)bl;
	sd2=va_arg(ap,MapSessionData *);
 ...
 */
MapSessionData *pc_get_father (MapSessionData *sd){
 ...
 */
MapSessionData *pc_get_mother (MapSessionData *sd){
 ...
 *------------------------------------------*/
MapSessionData *pc_get_child (MapSessionData *sd)
{
 ...
 *------------------------------------------*/
void pc_bleeding (MapSessionData *sd, t_tick diff_tick)
{
	int hp = 0, sp = 0;
...
//&1: HP regen
//&2: SP regen
void pc_regen (MapSessionData *sd, t_tick diff_tick)
{
	int hp = 0, sp = 0;
 ...
 *------------------------------------------*/
void pc_setsavepoint(MapSessionData *sd, short mapindex,int x,int y)
{
	nullpo_retv(sd);
...
static int pc_daynight_timer_sub(MapSessionData *sd,va_list ap)
{
...
*/
bool pc_setstand(MapSessionData *sd, bool force){
	nullpo_ret(sd);
 ...
 **/
void pc_overheat(MapSessionData &sd, int16 heat) {
	StatusChangeEntry *sce = sd.sc.getSCE(SC_OVERHEAT_LIMITPOINT);
 ...
 */
bool pc_isautolooting(MapSessionData *sd, t_itemid nameid)
{
	uint8 i = 0;
 ...
 */
bool pc_can_use_command( MapSessionData *sd, const char *command, AtCommandType type ){
	return sd->group->can_use_command( command, type );
}

bool pc_has_permission( MapSessionData* sd, e_pc_permission permission ){
 ...
 * @param sd Player map session data
 */
bool pc_should_log_commands( MapSessionData *sd ){
 ...
 */
void pc_addspiritcharm(MapSessionData *sd, int interval, int max, int type)
{
	int tid, i;
 ...
 */
void pc_delspiritcharm(MapSessionData *sd, int count, int type)
{
	int i;
 ...
 * @return Penalty rate
 */
uint16 pc_level_penalty_mod( MapSessionData* sd, e_penalty_type type, std::shared_ptr<mobs::s_mob_db> mob, mobs::MobData* md ){
...
void pc_itemcd_do(MapSessionData *sd, bool load) {
	int i,cursor = 0;
 ...
 **/
uint8 pc_itemcd_add(MapSessionData *sd, struct ItemData *id, t_tick tick, unsigned short n) {
	int i;
 ...
 *         1: Has delay, cancel consumption.
 **/
uint8 pc_itemcd_check(MapSessionData *sd, struct ItemData *id, t_tick tick, unsigned short n) {
...
**/
void pc_damage_log_add(MapSessionData *sd, int id)
{
	uint8 i = 0;
...
**/
void pc_damage_log_clear(MapSessionData *sd, int id)
{
	uint8 i;
 ...
 * @param sd: Player data
 */
void pc_scdata_received(MapSessionData *sd) {
 ...
 * @param sd: Player data
 */
void pc_check_expiration(MapSessionData *sd) {
...
void pc_expire_check(MapSessionData *sd) {  
	/* ongoing timer */
...
**/
enum e_BANKING_DEPOSIT_ACK pc_bank_deposit(MapSessionData *sd, int money) {
...
**/
enum e_BANKING_WITHDRAW_ACK pc_bank_withdraw(MapSessionData *sd, int money) {
	if (battle_config.feature_banking_state_enforce && !sd->state.banking) {
...
* @param sd: Player
**/
void pc_crimson_marker_clear(MapSessionData *sd) {
	uint8 i;
...
* @param sd: Player
**/
void pc_show_version(MapSessionData *sd) {
 ...
 * @author [Cydh]
 **/
void pc_bonus_script(MapSessionData *sd) {
	t_tick now = gettick();
 ...
 * @author [Cydh]
 **/
struct s_bonus_script_entry *pc_bonus_script_add(MapSessionData *sd, const char *script_str, t_tick dur, enum efst_type icon, uint16 flag, uint8 type) {
...
* @author [Cydh]
**/
void pc_bonus_script_free_entry(MapSessionData *sd, struct s_bonus_script_entry *entry) {
	if (entry->tid != INVALID_TIMER)
 ...
 * @param sd
 **/
static void inline pc_bonus_script_check_final(MapSessionData *sd) {
	if (sd->bonus_script.count == 0) {
...
* @author [Cydh]
**/
void pc_bonus_script_clear(MapSessionData *sd, uint32 flag) {
	struct linkdb_node *node = NULL;
	uint16 count = 0;
 ...
 * @param sd: Target player
 */
void pc_cell_basilica(MapSessionData *sd) {
	nullpo_retv(sd);
 ...
 * @return max_param
 */
uint16 pc_maxparameter(MapSessionData *sd, e_params param) {
	nullpo_retr(0, sd);
...
* @return ASPD
*/
short pc_maxaspd(MapSessionData *sd) {
	nullpo_ret(sd);
...
* @return Heal rate
**/
short pc_get_itemgroup_bonus(MapSessionData* sd, t_itemid nameid, std::vector<ItemBonus>& bonuses) {
	if (bonuses.empty())
		return 0;
...
* @return Heal rate
**/
short pc_get_itemgroup_bonus_group(MapSessionData* sd, uint16 group_id, std::vector<ItemBonus>& bonuses) {
	if (bonuses.empty())
		return 0;
 ...
 */
uint64 pc_generate_unique_id(MapSessionData *sd) {
	nullpo_ret(sd);
 ...
 * @param sd
 **/
void pc_validate_skill(MapSessionData *sd) {
	if (sd) {
		uint16 i = 0, count = 0;
 ...
 * @param sd Player
 **/
void pc_show_questinfo(MapSessionData *sd) {
 ...
 * @param sd Player
 **/
void pc_show_questinfo_reinit(MapSessionData *sd) {
#if PACKETVER >= 20090218
	nullpo_retv(sd);
 ...
 * @param sd
 **/
void pc_set_costume_view(MapSessionData *sd) {
...
int32 pc_attendance_counter( MapSessionData* sd ){
...
void pc_attendance_claim_reward( MapSessionData* sd ){
 ...
 */
void pc_jail(MapSessionData &sd, int32 duration) {
 ...
 */
static void pc_macro_punishment(MapSessionData &sd, e_macro_detect_status stype) {
	int32 duration = 0;
 ...
 */
void pc_macro_captcha_register(MapSessionData &sd, uint16 image_size, char captcha_answer[CAPTCHA_ANSWER_SIZE]) {
 ...
 */
void pc_macro_captcha_register_upload(MapSessionData &sd, uint16 upload_size, char *upload_data) {
 ...
 */
void pc_macro_detector_process_answer(MapSessionData &sd, char captcha_answer[CAPTCHA_ANSWER_SIZE]) {
	nullpo_retv(captcha_answer);
 ...
 * @param sd: Player data
 */
void pc_macro_detector_disconnect(MapSessionData &sd) {
 ...
 */
int pc_macro_reporter_area_select_sub(BlockList *bl, va_list ap) {
	nullpo_retr(0, bl);
 ...
 * @param radius: Area
 */
void pc_macro_reporter_area_select(MapSessionData &sd, const int16 x, const int16 y, const int8 radius) {
 ...
 */
void pc_macro_reporter_process(MapSessionData &sd, int32 reporter_account_id) {
	if (captcha_db.empty())
		return;
...
void do_init_pc(void) {
	...
	pc_sc_display_ers = ers_new(sizeof(struct ScDisplayEntry), "pc.cpp:pc_sc_display_ers", ERS_OPT_FLEX_CHUNK);
