...
int32 battle_gettarget(const block_list* bl)
{
	...
	switch (bl->type) {
		case BL_PC:  return reinterpret_cast<const map_session_data*>(bl)->ud.target;
		case BL_MOB: return reinterpret_cast<const mob_data*>(bl)->target_id;
		case BL_PET: return reinterpret_cast<const pet_data*>(bl)->target_id;
		case BL_HOM: return reinterpret_cast<const homun_data*>(bl)->ud.target;
		case BL_MER: return reinterpret_cast<const s_mercenary_data*>(bl)->ud.target;
		case BL_ELEM: return reinterpret_cast<const s_elemental_data*>(bl)->ud.target;
	}
 ...
 * @return Target list
 */
block_list* battle_getenemyarea(const block_list *src, int32 x, int32 y, int32 range, int32 type, int32 ignore_id)
{
	block_list* bl_list[MAX_ENEMY_SEARCH_COUNT];
	int32 c = 0;
...
static int32 battle_range_type(const block_list *src, const block_list *target, uint16 skill_id, uint16 skill_lv)
{
	...
	switch (skill_id) {
		...
		case CD_PETITIO: { // Skill range is 2 but damage is melee with books and ranged with mace.
			const map_session_data* sd = reinterpret_cast<const map_session_data*>(src);
...
bool is_infinite_defense(const block_list *target, int32 flag)
{
	...
	if(target->type == BL_SKILL) {
		const skill_unit* su = reinterpret_cast<const skill_unit*>(target);
...
static bool is_attack_left_handed( const block_list* src, int32 skill_id ){
	if(src != nullptr) {
		//Skills ALWAYS use ONLY your right-hand weapon (tested on Aegis 10.2)
		if(!skill_id) {
			const map_session_data* sd = static_cast<const map_session_data*>(src);
...
#endif
			case MH_SONIC_CRAW:{
				const TBL_HOM *hd = reinterpret_cast<homun_data*>(src);
				wd.div_ = hd->homunculus.spiritball;
			}
				break;
			case MO_FINGEROFFENSIVE:
...
{
	const status_data* tstatus = status_get_status_data(target);
	const mob_data* dstmd = reinterpret_cast<const mob_data*>(&target);
...
static bool battle_get_exception_ai( const block_list &src) {
	const mob_data* md = reinterpret_cast<const mob_data*>(&src);
...
int32 battle_check_target( const block_list *src, const block_list *target, int32 flag )
{
	...
	if ( s_bl->type == BL_PC ) {
		switch( t_bl->type ) {
			case BL_MOB: // Source => PC, Target => MOB
				if ( pc_has_permission(reinterpret_cast<const map_session_data*>(s_bl), PC_PERM_DISABLE_PVM) )
					return 0;
				break;
			case BL_PC:
				if (pc_has_permission(reinterpret_cast<const map_session_data*>(s_bl), PC_PERM_DISABLE_PVP))
					return 0;
				break;
	...
	switch( target->type ) { // Checks on actual target
		case BL_PC: {
				...
				if (reinterpret_cast<const map_session_data*>(target)->invincible_timer != INVALID_TIMER || pc_isinvisible(reinterpret_cast<const map_session_data*>(target)))
		...
		case BL_MOB:
		{
			const mob_data *md = reinterpret_cast<const mob_data*>(target);
	...
	switch( t_bl->type ) { //Checks on target master
		case BL_PC: {
			...
			const map_session_data* sd = reinterpret_cast<const map_session_data*>(t_bl);
		...
		case BL_MOB:
		{
			const mob_data *md = reinterpret_cast<const mob_data*>(t_bl);
	...
	switch( src->type ) { //Checks on actual src type
		case BL_PET:
			...
			break;
		case BL_SKILL: {
				const skill_unit* su = reinterpret_cast<const skill_unit*>(src);
			...
			break;
		case BL_MER:
			if (t_bl->type == BL_MOB && reinterpret_cast<const mob_data*>(t_bl)->mob_id == MOBID_EMPERIUM && flag&BCT_ENEMY)
	...
	{	//Checks on source master
		case BL_PC:
		{
			const map_session_data* sd = reinterpret_cast<const map_session_data*>(s_bl);
			if( s_bl != t_bl )
			{
				if( sd->state.killer )
				{
				...
				{
					if( t_bl->type == BL_PC && (sd->duel_group == reinterpret_cast<const map_session_data*>(t_bl)->duel_group) )
						return (BCT_ENEMY&flag)?1:-1; // Duel targets can ONLY be your enemy, nothing else.
					else
						return 0; // You can't target anything out of your duel
				}
			}
			if( !sd->status.guild_id && t_bl->type == BL_MOB && reinterpret_cast<const mob_data*>(t_bl)->mob_id == MOBID_EMPERIUM && mapdata_flag_gvg(mapdata) )
		...
		case BL_MOB:
		{
			const mob_data *md = reinterpret_cast<const mob_data*>(s_bl);
			if( md->guardian_data && md->guardian_data->guild_id && !mapdata_flag_gvg(mapdata) )
			...
			{ //Normal mobs
				if(
					...
					( t_bl->type == BL_MOB && (reinterpret_cast<const mob_data*>(t_bl)->special_state.ai == AI_NONE || reinterpret_cast<const mob_data*>(t_bl)->special_state.ai == AI_WAVEMODE ))
				  )
			...
			{
				if( t_bl->type == BL_MOB && !(reinterpret_cast<const mob_data*>(t_bl)->special_state.ai ) )
	...
	{ //Check rivalry settings.
		...
		{ // Prevent novice engagement on pk_mode (feature by Valaris)
			const map_session_data* sd = reinterpret_cast<const map_session_data*>(s_bl);
			const map_session_data* sd2 = reinterpret_cast<const map_session_data*>(t_bl);
			if (
