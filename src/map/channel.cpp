...
int channel_delete(struct Channel *channel, bool force) {
	...
	if( db_size(channel->users)) {
		...
		for( sd = (MapSessionData *)dbi_first(iter); dbi_exists(iter); sd = (MapSessionData *)dbi_next(iter) ) { //for all users
 ...
 *  -4: Reached max limit
 */
int channel_join(struct Channel *channel, MapSessionData *sd) {
	if(!channel || !sd)
		return -1;
 ...
 */
int channel_mjoin(MapSessionData *sd) {
	char mout[60];
	if(!sd) return -1;
 ...
 */
int channel_gjoin(MapSessionData *sd, int flag){
	struct Channel *channel;
 ...
 */
int channel_clean(struct Channel *channel, MapSessionData *sd, int flag) {
	unsigned char i;
 ...
 * -1: Invalid player
 */
int channel_pcquit(MapSessionData *sd, int type){
	int i;
 ...
 */
int channel_send(struct Channel *channel, MapSessionData *sd, const char *msg) {
	int idx = 0;
 ...
 */
struct Channel* channel_name2channel(char *chname, MapSessionData *sd, int flag){
 ...
 */
int channel_haspc(struct Channel *channel,MapSessionData *sd){
 ...
 *  1: Player is banned
 */
int channel_haspcbanned(struct Channel *channel,MapSessionData *sd){
	if(!channel || !sd) return -1;
 ...
 */
int channel_pc_haschan(MapSessionData *sd, struct Channel *channel){
	int k;
 ...
 */
int channel_display_list(MapSessionData *sd, const char *options){
 ...
 * @return 0 on success or -1 on failure
 */
int channel_pccreate(MapSessionData *sd, char *chname, char *chpass){
	char output[CHAT_SIZE_MAX];
	int8 res;
 ...
 * @return 0 on success or -1 on failure
 */
int channel_pcdelete(MapSessionData *sd, char *chname){
	struct Channel *channel;
	char output[CHAT_SIZE_MAX];
 ...
 * @return 0 on success or -1 on failure
 */
int channel_pcleave(MapSessionData *sd, char *chname){
	struct Channel *channel;
	char output[CHAT_SIZE_MAX];
 ...
 * @return 0 on success or -1 on failure
 */
int channel_pcjoin(MapSessionData *sd, char *chname, char *pass){
	struct Channel *channel;
	char output[CHAT_SIZE_MAX];
 ...
 * @return 0 on success or -1 on failure
 */
int channel_pccolor(MapSessionData *sd, char *chname, char *color){
	struct Channel *channel;
	char output[CHAT_SIZE_MAX];
	int k;
 ...
 */
int channel_pcbind(MapSessionData *sd, char *chname){
	struct Channel *channel;
	char output[CHAT_SIZE_MAX];
 ...
 * @return 0 on success or -1 on failure
 */
int channel_pcunbind(MapSessionData *sd){
	char output[CHAT_SIZE_MAX];
 ...
 * @return 0 on success or -1 on failure
 */
int channel_pcban(MapSessionData *sd, char *chname, char *pname, int flag){
	struct Channel *channel;
	char output[CHAT_SIZE_MAX];
	MapSessionData *tsd = map_nick2sd(pname,false);
 ...
 * @return 0 on success or -1 on failure
 */
int channel_pckick(MapSessionData *sd, char *chname, char *pname) {
	struct Channel *channel;
	char output[CHAT_SIZE_MAX];
	MapSessionData *tsd = map_nick2sd(pname,false);
 ...
 * @return 0 on success or -1 on failure
 */
int channel_pcsetopt(MapSessionData *sd, char *chname, const char *option, const char *val){
	struct Channel *channel;
	char output[CHAT_SIZE_MAX];
	int k, s = 0, opt;
	const char* opt_str[] = {
...
int channel_pcautojoin_sub(DBKey key, DBData *data, va_list ap) {
	...
	nullpo_ret(channel);
	nullpo_ret((sd = va_arg(ap, MapSessionData *)));
 ...
 * @param sd: Player data
 */
void channel_autojoin(MapSessionData *sd) {
	nullpo_retv(sd);
