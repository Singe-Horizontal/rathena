 ...
 */
MapSessionData *inter_search_sd(uint32 account_id, uint32 char_id)
{
 ...
 * @return 
 */
int intif_rename(MapSessionData *sd, int type, char *name)
{
	if (CheckForCharServer())
		return 0;
 ...
 * @return 
 */
int intif_main_message(MapSessionData* sd, const char* message)
{
	char output[256];
 ...
 */
int intif_wis_message(MapSessionData *sd, char *nick, char *mes, int mes_len)
{
 ...
 */
int intif_saveregistry(MapSessionData *sd)
{
	DBIterator *iter;
	DBKey key;
	DBData *data;
 ...
 * @return 
 */
int intif_request_registry(MapSessionData *sd, int flag)
{
	nullpo_ret(sd);
 ...
 * @return 0=error, 1=msg sent
 */
int intif_party_changemap(MapSessionData *sd,int online)
{
	int16 m, mapindex;
...
int intif_parse_WisEnd(int fd)
{
	...
	sd = (MapSessionData *)map_nick2sd(RFIFOCP(fd,2),false);
	if (sd != NULL)
 ...
 * @return 0=error, 1=msg sent
 */
static int mapif_parse_WisToGM_sub(MapSessionData* sd,va_list va)
{
 ...
 */
void intif_request_questlog(MapSessionData *sd)
{
	if (CheckForCharServer())
		return;
 ...
 */
int intif_quest_save(MapSessionData *sd)
{
...
{
	int cid = RFIFOL(fd, 2);
	MapSessionData *sd = map_charid2sd(cid);
 ...
 * @return 0 in case of success, nonzero otherwise
 */
int intif_achievement_save(MapSessionData *sd)
{
...
void intif_parse_achievementreward(int fd){
	MapSessionData *sd = map_charid2sd(RFIFOL(fd,2));
 ...
 */
int intif_achievement_reward(MapSessionData *sd, struct s_achievement_db *adb){
	if( CheckForCharServer() ){
		return 0;
	}
 ...
 * @return 0=error, 1=msg sent
 */
bool intif_mail_getattach( MapSessionData* sd, struct mail_message *msg, enum mail_attachment_type type){
	if (CheckForCharServer())
		return false;
...
bool intif_mail_checkreceiver( MapSessionData* sd, char* name ){
	MapSessionData *tsd;
...
{
	MapSessionData *sd = map_charid2sd(RFIFOL(fd,4));
...
void intif_parse_MessageToFD(int fd) {
	...
	if( session[u_fd] && session[u_fd]->session_data ) { //check if the player still online
		int aid = RFIFOL(fd,8);
		MapSessionData * sd = (MapSessionData *)session[u_fd]->session_data;
 ...
 * @return
 **/
int intif_broadcast_obtain_special_item(MapSessionData *sd, t_itemid nameid, t_itemid sourceid, unsigned char type) {
	nullpo_retr(0, sd);
 ...
 * @return
 **/
int intif_broadcast_obtain_special_item_npc(MapSessionData *sd, t_itemid nameid) {
	nullpo_retr(0, sd);
...
void intif_parse_itembound_store2gstorage(int fd) {
	...
	for (i = 0; i < count; i++) {
		Item *item = (struct Item*)RFIFOP(fd, 8 + i*sizeof(struct Item));
		if (!item)
			continue;
 ...
 * @return false - error, true - message sent
 */
bool intif_storage_request(MapSessionData *sd, enum storage_type type, uint8 stor_id, uint8 mode)
{
	if (CheckForCharServer())
		return false;
 ...
 * @ return false - error, true - message sent
 */
bool intif_storage_save(MapSessionData *sd, struct s_storage *stor)
{
