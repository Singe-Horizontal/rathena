 ...
 */
static bool achievement_done(MapSessionData *sd, int achievement_id) {
 ...
 */
bool achievement_check_dependent(MapSessionData *sd, int achievement_id)
{
	nullpo_retr(false, sd);
 ...
 */
static int achievement_check_groups(MapSessionData *sd, struct s_achievement_db *ad)
{
	int i;
 ...
 */
static bool achievement_update_objectives(MapSessionData *sd, std::shared_ptr<struct s_achievement_db> ad, enum e_achievement_group group, const std::array<int, MAX_ACHIEVEMENT_OBJECTIVES> &update_count)
{
	if (!ad || !sd)
		return false;
